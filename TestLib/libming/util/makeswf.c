...
FileType getFileType(char *filename)
{
	...
	if (!strcasecmp(ext, ".swf"))
		return SWF;
	if (!strcasecmp(ext, ".png"))
		return BITMAP;
	if (!strcasecmp(ext, ".dbl"))
		return BITMAP;
...
#define BUFSIZE 1024
	...
	mo = newSWFMovie();
	if (networkAccess >= 0)
		SWFMovie_setNetworkAccess(mo, networkAccess);
...
add_init_action(char *file, int frameno)
{
	...
	if (frameno > max_init_action_frame)
		max_init_action_frame = frameno;
}
